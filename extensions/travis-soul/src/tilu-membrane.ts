...
const TRUTH_VIOLATIONS = [/definitely|certainly|always|never|guaranteed/i, /100%|absolutely sure/i];

const INTEGRITY_VIOLATIONS = [/don't worry about/i, /just trust me/i];

export async function tiluMembrane(content: string, _soul: TravisSoul): Promise<MembraneResult> {
  const flags: string[] = [];
